"""
validate_online_val_knn.py
--------------------------
Validate an online PPO residual policy on the validation split carved from
`Data/train` using the exact same split logic as `train.py`, while matching
the Euclidean nearest-k collision-reward environment used by
...
from online_inference import resolve_policy_metadata, valid_indices
from online_inference_knn import evaluate_online_policy
...
from validate_online_val import write_results


def parse_args():
    parser = argparse.ArgumentParser(
        description="Validate an online PPO model with the kNN-reward environment on the train.py validation split."
    )
    parser.add_argument(
        "--model",
        required=True,
