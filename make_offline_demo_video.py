"""
make_offline_demo_video.py
--------------------------
Create a short demo video for a frozen offline trajectory prior using the same
synthetic-scene setup as the online demo. This is a reference video with no
online PPO residual policy applied.

Example
-------
    python make_offline_demo_video.py \
        --model transformer \
        --prior_checkpoint checkpoints/transformer.pth \
        --output_file outputs/offline_transformer_demo.mp4 \
        --device cuda
"""
...
from make_online_demo_video import (
    apply_total_frames,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Create a synthetic offline-prior demo video."
    )
    parser.add_argument(
        "--model",
        default="transformer",
    ...
    parser.add_argument("--output_file", default="outputs/offline_transformer_demo.mp4")
    return parser.parse_args()


def load_prior_env(args, device: torch.device):
    checkpoint = torch.load(args.prior_checkpoint, map_location="cpu")
...
@torch.no_grad()
def rollout_prior_only(env: PedPyPriorEnv, scene: torch.Tensor) -> torch.Tensor:
    env.reset(scene)
    predictions = []
    done = False
...
def build_demo_trajectories(env, args, device: torch.device):
    scene = build_synthetic_scene(args.obs_length, args.pred_length, args.dt, args.num_neighbors, args.seed).to(device)
    if args.fixed_neighbors:
        pred_future = rollout_prior_only_fixed_neighbors(env, scene).detach().cpu()
    else:
        pred_future = rollout_prior_only(env, scene).detach().cpu()
    observed = scene[: args.obs_length].detach().cpu()
    return torch.cat([observed, pred_future], dim=0)


@torch.no_grad()
def rollout_prior_only_fixed_neighbors(env: PedPyPriorEnv, scene: torch.Tensor) -> torch.Tensor:
    env.reset(scene)
    ...
    while env.step_idx < env.pred_length:
        ...
        candidate_vel = prior_velocities
...
def render_frame(
    ...
    phase = "Observed" if frame_idx < obs_length else "Offline Prior Rollout"
    ax.set_title(f"Transformer Prior Only | {phase} | Frame {frame_idx + 1}/{traj.shape[0]}")
    ax.set_xlabel("x (m)")
    ax.set_ylabel("y (m)")
    ...
    note = (
        ...
        "Future ego uses transformer prior only.\n"
...
def main():
    ...
    env = load_prior_env(args, device)
    traj = build_demo_trajectories(env, args, device)
